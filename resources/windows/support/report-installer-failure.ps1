...
function Read-RickyTicky BobbyWobbinAnalyticsId {
  $candidates = @()
  if ($env:APPDATA) {
    $candidates += (Join-Path $env:APPDATA 'RickyTicky BobbyWobbin\analytics.json')
  }
  if ($env:LOCALAPPDATA) {
    $candidates += (Join-Path $env:LOCALAPPDATA 'RickyTicky BobbyWobbin\analytics.json')
  }
...
function Show-ReportMessage([string]$text, [string]$icon) {
  ...
  [System.Windows.Forms.MessageBox]::Show(
    $text,
    'RickyTicky BobbyWobbin installer report',
...
function New-ReportDetailsText(
  ...
  $lines.Add('--------------------------------')
  $lines.Add('RickyTicky BobbyWobbin installer failure ' + $code)
  $lines.Add('--------------------------------')
  $lines.Add('')
  if ($eventId) {
  ...
  $lines.Add('---------------------------')
  $lines.Add('To RickyTicky BobbyWobbin Team')
  $lines.Add('---------------------------')
...
try {
  ...
  $userId = Read-RickyTicky BobbyWobbinAnalyticsId
  $eventPayload = @{
  ...
  Show-ReportMessage ('RickyTicky BobbyWobbin installer report sent.' + [Environment]::NewLine + [Environment]::NewLine + 'Tip: If you can get in touch with the RickyTicky BobbyWobbin team, press [ Ctrl + C ] in this dialog to copy details, then send them to us via social media, email, or a GitHub issue:' + [Environment]::NewLine + 'https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues' + [Environment]::NewLine + [Environment]::NewLine + $reportDetails) 'Information'
} catch {
  ...
  Show-ReportMessage ('RickyTicky BobbyWobbin installer report failed.' + [Environment]::NewLine + [Environment]::NewLine + 'Status: ' + $statusPath + [Environment]::NewLine + 'Log: ' + $log) 'Exclamation'
}
