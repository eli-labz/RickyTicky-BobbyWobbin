...
if (!gotTheLock) {
  console.warn('[RickyTicky BobbyWobbin] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[RickyTicky BobbyWobbin] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[RickyTicky BobbyWobbin] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[RickyTicky BobbyWobbin] runBackendMigrations completed');
    } catch (error) {
      console.error('[RickyTicky BobbyWobbin] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[RickyTicky BobbyWobbin] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  ...
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[RickyTicky BobbyWobbin] Ignoring AIONUI_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[RickyTicky BobbyWobbin] Ignoring unknown AIONUI_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[RickyTicky BobbyWobbin] Creating main window...');
  ...
  console.log(`[RickyTicky BobbyWobbin] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[RickyTicky BobbyWobbin] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[RickyTicky BobbyWobbin] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[RickyTicky BobbyWobbin] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[RickyTicky BobbyWobbin] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[RickyTicky BobbyWobbin] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[RickyTicky BobbyWobbin] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[RickyTicky BobbyWobbin] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[RickyTicky BobbyWobbin] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[RickyTicky BobbyWobbin] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[RickyTicky BobbyWobbin] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[RickyTicky BobbyWobbin] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[RickyTicky BobbyWobbin] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[RickyTicky BobbyWobbin] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[RickyTicky BobbyWobbin] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[RickyTicky BobbyWobbin] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[RickyTicky BobbyWobbin] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[RickyTicky BobbyWobbin:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  if (process.platform === 'darwin' && !app.isPackaged && app.dock) {
    try {
      const iconPath = path.join(process.cwd(), 'resources', 'app.png');
      if (fs.existsSync(iconPath)) {
  ...
  try {
    ...
    console.error('[RickyTicky BobbyWobbin] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[RickyTicky BobbyWobbin] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`RickyTicky BobbyWobbin --webui` or packaged `aionui-web` mode that
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[RickyTicky BobbyWobbin] App initialization failed:', error);
    app.quit();
  });
...
app.on('will-quit', () => {
  console.log('[RickyTicky BobbyWobbin] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[RickyTicky BobbyWobbin] quit (exitCode=${exitCode})`);
});
