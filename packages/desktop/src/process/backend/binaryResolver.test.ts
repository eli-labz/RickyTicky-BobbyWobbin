    ...
    try {
      ...
      expect(error).toMatchObject({
        ...
        diagnostics: expect.objectContaining({
          ...
          checkedFallbackPaths: expect.any(Array),
        }),
      });
    }
  });

  it('resolves from AIONCORE_BINARY when PATH lookup fails', () => {
    const binaryPath = process.platform === 'win32' ? 'C:/custom/aioncore.exe' : '/custom/aioncore';

    process.env.AIONCORE_BINARY = binaryPath;
