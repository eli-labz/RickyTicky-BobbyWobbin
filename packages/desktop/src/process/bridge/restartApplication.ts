...
export function restartApplication(app: RestartableApp): IAppRestartResult {
  if (!app.isPackaged) {
    console.info('[RickyTicky BobbyWobbin] Restart skipped in development mode; manual restart required');
    return {
      restarted: false,
  ...
  console.info('[RickyTicky BobbyWobbin] Relaunching application to apply changes');
  app.relaunch();
  app.exit(0);
  return {
    restarted: true,
