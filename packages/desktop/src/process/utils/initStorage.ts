...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[RickyTicky BobbyWobbin] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[RickyTicky BobbyWobbin] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[RickyTicky BobbyWobbin] 数据迁移失败:', error);
  }
...
const cleanupLegacyBuiltinSkillsDir = () => {
  ...
  fs.rm(legacyDir, { recursive: true, force: true })
    .then(() => console.log('[RickyTicky BobbyWobbin] Cleaned up legacy builtin-skills cache'))
    .catch(() => {
...
const initStorage = async () => {
  ...
  const mark = (label: string) => console.log(`[RickyTicky BobbyWobbin:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[RickyTicky BobbyWobbin] Failed to ensure assistant dirs:', error);
  }
