...
/** Backend manifest id of the built-in RickyTicky BobbyWobbin Butler assistant. */
 ...
 * select the RickyTicky BobbyWobbin Butler, and pre-fill the chat input with a ready-made
      ...
      try {
        ...
        if (butler) {
          ...
          if (butler.enabled === false) {
            ...
            Message.success(
              t('settings.talkToButler.enabledToast', { defaultValue: 'Enabled the RickyTicky BobbyWobbin Butler for you' })
            );
          }
        }
      } catch (error) {
