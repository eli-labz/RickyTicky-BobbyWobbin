...
import { normalizeLegacyProductCopy, resolveAssistantName } from '@/renderer/utils/model/assistantDisplay';
...
const resolveLocalizedRecommendedPrompts = (
  ...
  return (
    ...
    []
  ).map((prompt) => normalizeLegacyProductCopy(prompt));
};
...
const resolveLocalizedProfileField = (
  ...
  localeKey: string,
  fallbackValue = ''
): string => normalizeLegacyProductCopy(localizedValues?.[localeKey] ?? localizedValues?.['en-US'] ?? baseValue ?? fallbackValue);
...
export const useAssistantEditor = ({
  ...
  useEffect(() => {
    ...
    void loadAssistantDetail(activeAssistant.id)
      .then((detail) => {
        ...
        setEditContext(normalizeLegacyProductCopy(detail.rules.content || ''));
  ...
  const handleEdit = async (assistant: AssistantListItem) => {
    ...
    setEditName(resolveAssistantName(assistant, localeKey, assistant.name || ''));
    setEditDescription(normalizeLegacyProductCopy(assistant.description || ''));
  ...
  const handleDuplicate = async (assistant: AssistantListItem) => {
    ...
    setEditVisible(true);
    setPromptViewMode('edit');
    setEditName(`${resolveAssistantName(assistant, localeKey, assistant.name)} (Copy)`);
    setEditDescription(normalizeLegacyProductCopy(assistant.description_i18n?.[localeKey] || assistant.description || ''));
