...
import { normalizeLegacyProductCopy } from '@/renderer/utils/model/assistantDisplay';
...
const HIDDEN_SKILL_NAMES = new Set(['xiaohongshu-recruiter']);
...
const LEGACY_SKILL_NAME_PATTERN = /aionui/gi;
const CHINESE_TEXT_PATTERN = /\p{Script=Han}/u;

const getSkillDisplayName = (name: string): string => name.replace(LEGACY_SKILL_NAME_PATTERN, 'rickyticky-bobbywobbin');

const getSkillDisplayDescription = (description: string | undefined): string => {
  const normalized = normalizeLegacyProductCopy(description || '');
  return CHINESE_TEXT_PATTERN.test(normalized) ? '' : normalized;
};
...
const isVisibleSkill = (skill: SkillInfo): boolean => !HIDDEN_SKILL_NAMES.has(skill.name);
...
const SkillsHubSettings: React.FC<SkillsHubSettingsProps> = ({ withWrapper = true }) => {
  ...
  const officialSkills = useMemo(
    () => availableSkills.filter((s) => s.source === 'builtin' && !s.is_auto_inject && isVisibleSkill(s)),
    [availableSkills]
  );
  const builtinAutoSkills = useMemo(
    () => availableSkills.filter((skill) => isAutoInjectedBuiltinSkill(skill) && isVisibleSkill(skill)),
    [availableSkills]
  );
  const extensionSkills = useMemo(
    () => availableSkills.filter((s) => s.source === 'extension' && isVisibleSkill(s)),
    [availableSkills]
  );
  ...
  const filteredSkills = useMemo(() => matchesQuery(mySkills.filter(isVisibleSkill)), [matchesQuery, mySkills]);
  ...
  const renderReadonlySkillCard = (skill: SkillInfo, variant: 'official' | 'extension' | 'auto', testId?: string) => {
    ...
    const displayDescription = getSkillDisplayDescription(skill.description);
    return (
      <div
        key={skill.name}
        data-testid={testId}
      ...
      >
        ...
        <div className='flex-1 min-w-0 flex flex-col justify-center gap-4px'>
          <h3 className='text-14px font-semibold text-t-primary/90 truncate m-0'>{displayName}</h3>
          {displayDescription && (
            <p className='text-13px text-t-secondary leading-relaxed line-clamp-2 m-0' title={displayDescription}>
              {displayDescription}
            </p>
          )}
        </div>
                  ...
                  onClick={() => {
                    Modal.confirm({
                      ...
                      content: t('settings.skillsHub.deleteConfirmContent', {
                        name: displayName,
                        defaultValue: `Are you sure you want to delete "${displayName}"?`,
                      }),
      ...
      <div data-testid='official-skills-section'>
        <p className='m-0 mb-12px text-12px leading-relaxed text-t-tertiary'>
          {t('settings.skillsHub.officialHint', {
            defaultValue: 'Built-in skills maintained by RickyTicky BobbyWobbin — read-only and updated with each release.',
          })}
        </p>
