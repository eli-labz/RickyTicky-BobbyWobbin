...
// Read the real RickyTicky BobbyWobbin version from the repo-root package.json.
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another RickyTicky BobbyWobbin clone is running),
      ...
      define: {
        ...
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        // Inject the real RickyTicky BobbyWobbin version (root package.json) so renderer code
