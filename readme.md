...
<p align="center">
  <img src="https://img.shields.io/github/v/release/eli-labz/RickyTicky-BobbyWobbin?style=flat-square&color=32CD32" alt="RickyTicky BobbyWobbin version">
  &nbsp;
...
<p align="center">
  <a href="https://github.com/eli-labz/RickyTicky-BobbyWobbin/releases">
...
[Why Choose RickyTicky BobbyWobbin?](#-why-choose-aionui-over-claude-cowork) ·
...
**RickyTicky BobbyWobbin is more than a chat client.** It's a Cowork platform where AI agents work alongside you on your computer — reading files, writing code, browsing the web, and automating tasks. You see everything the agent does, and you're always in control.

|                                 | Traditional AI Chat Clients | **RickyTicky BobbyWobbin (Cowork)**                                                                                     |
| :------------------------------ | :-------------------------- | :---------------------------------------------------------------------------------------------------------------------- |
...
RickyTicky BobbyWobbin ships with a complete AI agent engine. Unlike tools that require you to install CLI agents separately, **RickyTicky BobbyWobbin works the moment you install it**.
...
If you already use Claude Code, Codex, Hermes Agent, or OpenClaw, RickyTicky BobbyWobbin auto-detects them and lets you Cowork with all of them — alongside the built-in agent.

**Supported Agents:** Built-in Agent (zero setup) • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Aion CLI (aionrs, the Rust-based backend service shipped with RickyTicky BobbyWobbin) • Snow CLI • Hermes Agent • Cursor Agent and more
...
- **Parallel multi-agent execution** — Leader breaks tasks into subtasks and delegates to Teammate agents running in parallel; each Teammate uses its own model via ACP (Agent Communication Protocol, RickyTicky BobbyWobbin's multi-agent coordination layer), Gemini, or Aionrs
...
Other AI apps give you a chatbox with your API key. **RickyTicky BobbyWobbin gives you a full Cowork agent.**
...
Same agent capabilities — file read/write, web search, image generation, tool use — regardless of which model powers it. RickyTicky BobbyWobbin supports **30+ AI platforms** including cloud services and local deployments.
...
RickyTicky BobbyWobbin also supports [NewAPI](https://github.com/QuantumNous/new-api) gateway service — a unified AI model hub that aggregates and distributes various LLMs. Flexibly switch between different models in the same interface to meet various task requirements.
...
- **Three-tier Skills** — Builtin skills (shipped with RickyTicky BobbyWobbin), custom skills (your own), and Extension skills (contributed by third-party extensions); enable/disable per conversation with the skill indicator
...
RickyTicky BobbyWobbin supports three skill layers: **built-in** skills (shipped with the app), **custom** skills (user-defined), and **extension** skills (loaded from the Extension SDK).
...
RickyTicky BobbyWobbin includes **21 professional assistants** with predefined capabilities, extendable through custom skills:
...
**Custom Skills**: Create skills in the `skills/` directory, enable/disable skills for assistants to extend AI capabilities. Skills come from three sources: builtin (shipped with RickyTicky BobbyWobbin), custom (your own), and Extension (contributed via the Extension SDK). Built-in skills include `pptx`, `docx`, `pdf`, `xlsx`, `mermaid`, and more.
...
_Your 24/7 AI assistant — access RickyTicky BobbyWobbin from any device, anywhere._
...
> **Setup:** RickyTicky BobbyWobbin Settings → WebUI Settings → Channel, configure the Bot Token.
...
> [Remote Internet Access Tutorial](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/Remote-Internet-Access-Guide-Chinese)
...
- **Keep-awake** — RickyTicky BobbyWobbin automatically prevents system sleep while tasks are active, and detects missed triggers after wake
...
> [Image generation model configuration guide](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/RickyTicky BobbyWobbin-Image-Generation-Tool-Model-Configuration-Guide)
...
## 🤔 Why Choose RickyTicky BobbyWobbin Over Claude Cowork?
...
RickyTicky BobbyWobbin is a **free and open-source Multi-AI Agent Desktop**. Compared to Claude Cowork which only runs on macOS and is locked to Claude, RickyTicky BobbyWobbin is its full-model, cross-platform enhanced version.

| Dimension     | Claude Cowork | RickyTicky BobbyWobbin                                                    |
| :------------ | :------------ | :-------------------------------------------------------- |
...
A: <strong>No.</strong> RickyTicky BobbyWobbin has a built-in AI agent that works immediately after installation. Just enter any API key to get started. If you also have CLI tools like Claude Code or Gemini CLI installed, RickyTicky BobbyWobbin will auto-detect and integrate them for even more capabilities.
</details>

<details>
<summary><strong>Q: What can I do with RickyTicky BobbyWobbin?</strong></summary>
A: RickyTicky BobbyWobbin is your <strong>private Cowork workspace</strong>. The built-in agent can batch organize folders, process Excel data, generate documents, search the web, and generate images. With Multi-Agent Mode, you can also leverage Claude Code, Codex, and other powerful CLI agents through the same interface.
</details>

<details>
<summary><strong>Q: Is it free?</strong></summary>
A: RickyTicky BobbyWobbin is completely free and open source. You only pay for the API usage of whichever provider you choose, and you can use API keys from any provider you prefer.
</details>

<details>
<summary><strong>Q: Can I run RickyTicky BobbyWobbin on a server (headless)?</strong></summary>
A: Yes — RickyTicky BobbyWobbin WebUI mode runs as a standalone HTTP server. See the WebUI section above for setup instructions.
</details>
...
## See How People Use RickyTicky BobbyWobbin
...
> **Made a video about RickyTicky BobbyWobbin?** [Let us know on X](https://x.com/RickyTicky BobbyWobbin) and we'll feature it here!
...
1. **Install** RickyTicky BobbyWobbin
...
- [📖 Complete Installation Guide](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/Getting-Started) — Detailed steps from download to configuration
- [⚙️ LLM Configuration Guide](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/LLM-Configuration) — Multi-platform AI model configuration
- [🤖 Multi-Agent Mode Setup](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/ACP-Setup) — Integrate terminal AI agents
- [🔌 MCP Tool Configuration](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/MCP-Configuration-Guide) — Model Context Protocol server setup
- [🌐 WebUI Configuration Guide](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/WebUI-Configuration-Guide) — Complete WebUI setup and configuration tutorial

**🎯 Use Cases**

- [📁 File Management](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/file-management) — Smart file organization
- [📊 Excel Processing](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/excel-processing) — AI-driven data processing
- [🎨 Image Generation](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/RickyTicky BobbyWobbin-Image-Generation-Tool-Model-Configuration-Guide) — AI image creation
- [📚 More Use Cases](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/Use-Cases-Overview)

**❓ Support & Help**

- [❓ FAQ](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/FAQ) — Questions and troubleshooting
- [🔧 Configuration & Usage Tutorials](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki/Configuration-Guides) — Complete configuration documentation
...
- [GitHub Discussions](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/discussions) — share ideas and exchange tips
- [Report Issues](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues) — bugs and feature requests
- [Release Updates](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases) — get the latest version
...
RickyTicky BobbyWobbin local development uses two repositories: RickyTicky BobbyWobbin for the Electron frontend and AionCore for the local backend. See the [Development Guide](docs/contributing/development.md) for the full macOS, Linux, and Windows setup.
...
[Report Bug](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues) · [Request Feature](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues)
