 ...
 *      (e.g. out/linux-unpacked/aionui, out/mac-arm64/RickyTicky BobbyWobbin.app, out/win-unpacked/RickyTicky BobbyWobbin.exe).
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/RickyTicky BobbyWobbin.exe  or  out/win-x64-unpacked/RickyTicky BobbyWobbin.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'RickyTicky BobbyWobbin.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/RickyTicky BobbyWobbin.app/Contents/MacOS/RickyTicky BobbyWobbin  or  out/mac/RickyTicky BobbyWobbin.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'RickyTicky BobbyWobbin');
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['aionui', 'RickyTicky BobbyWobbin']) {
