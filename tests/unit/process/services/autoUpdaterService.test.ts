...
describe('AutoUpdaterService', () => {
  ...
  it('does not use the stable CDN updater when prerelease manual mode is enabled', async () => {
    autoUpdaterMock.checkForUpdates.mockResolvedValue({
      ...
      updateInfo: {
        version: '2.1.14',
        files: [{ url: 'RickyTicky BobbyWobbin-2.1.14-mac-arm64.dmg', sha512: 'sha512-value' }],
        path: 'RickyTicky BobbyWobbin-2.1.14-mac-arm64.dmg',
        sha512: 'sha512-value',
  ...
  it('configures electron-updater to read stable metadata from the CDN', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenCalledWith({
      provider: 'custom',
      url: 'https://static.rickyticky-bobbywobbin.com/releases',
  ...
  it('restores a completed cached auto-update when the downloaded package validates', async () => {
    const updateInfo = {
      version: '2.1.14',
      files: [{ url: 'RickyTicky BobbyWobbin-2.1.14-mac.zip', sha512: 'sha512-value' }],
      path: 'RickyTicky BobbyWobbin-2.1.14-mac.zip',
      sha512: 'sha512-value',
      releaseDate: '2026-06-08T00:00:00.000Z',
    };
    const fileInfo = {
      url: new URL('https://static.rickyticky-bobbywobbin.com/releases/2.1.14/RickyTicky BobbyWobbin-2.1.14-mac.zip'),
      info: { url: 'RickyTicky BobbyWobbin-2.1.14-mac.zip', sha512: 'sha512-value' },
    };
    const cachedUpdatePath = path.join('/cache/pending', 'RickyTicky BobbyWobbin-2.1.14-mac.zip');
