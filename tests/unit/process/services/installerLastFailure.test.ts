...
describe('installerLastFailure service', () => {
  it('consumes a valid installer-last-failure marker once from the AppData RickyTicky BobbyWobbin directory', async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        blockers: [{ pid: 1234, name: 'RickyTicky BobbyWobbin.exe' }],
      })
    );
    ...
    expect(consumed?.blockers).toEqual([{ pid: 1234, name: 'RickyTicky BobbyWobbin.exe' }]);
