...
describe('CdnGenericProvider', () => {
  it('resolves relative update files under the version directory', () => {
    ...
    const files = provider.resolveFiles({
      ...
      files: [
        {
          url: 'RickyTicky BobbyWobbin-2.1.14-mac-arm64.dmg',
          sha512: 'sha512-value',
        },
      ],
    ...
    expect(files[0]?.url.href).toBe('https://static.rickyticky-bobbywobbin.com/releases/2.1.14/RickyTicky BobbyWobbin-2.1.14-mac-arm64.dmg');
  });
});
