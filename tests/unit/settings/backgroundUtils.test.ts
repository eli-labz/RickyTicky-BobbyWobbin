...
describe('backgroundUtils', () => {
  ...
  it('removes old block completely when replacing', () => {
    ...
    const startCount = (result.match(/\/\* RickyTicky BobbyWobbin Theme Background Start \*\//g) || []).length;
    const endCount = (result.match(/\/\* RickyTicky BobbyWobbin Theme Background End \*\//g) || []).length;
