...
function Wait-BuildProcess($Build, [int]$TimeoutSeconds) {
  ...
  $source = Join-Path $Build.worktreePath "out\RickyTicky BobbyWobbin-$($Build.version)-win-x64.exe"
  ...
  $target = Join-Path $script:OutputDirResolved "RickyTicky BobbyWobbin-$($Build.version)-win-x64.exe"
  Copy-Item -LiteralPath $source -Destination $target -Force
...
if (-not (Test-Path -LiteralPath (Join-Path $localAioncoreBundleDir 'managed-resources') -PathType Container)) {
  $localAioncoreBundleDir = Join-Path $env:LOCALAPPDATA 'Programs\RickyTicky BobbyWobbin\resources\bundled-aioncore\win32-x64'
}
...
try {
  foreach ($version in $buildVersions) {
    $worktreePath = Join-Path $runRoot "RickyTicky BobbyWobbin-$version"
