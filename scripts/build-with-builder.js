...
/**
 * Simplified build script for RickyTicky BobbyWobbin
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('RickyTicky BobbyWobbin.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running RickyTicky BobbyWobbin/Electron process. Attempting to close...');
        killWindowsProcesses(['RickyTicky BobbyWobbin.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running RickyTicky BobbyWobbin/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'RickyTicky BobbyWobbin.exe');
    ...
    console.log('⚠️  Windows local build failed after RickyTicky BobbyWobbin.exe was produced.');
    if (firstError) {
