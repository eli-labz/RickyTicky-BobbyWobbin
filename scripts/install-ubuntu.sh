#!/usr/bin/env bash
# ============================================================================
# RickyTicky BobbyWobbin — Ubuntu / Debian 一鍵自動化安裝腳本
# ============================================================================
# 功能：
...
#   5. 建立服務管理腳本 (/opt/RickyTicky BobbyWobbin/start-aionui.sh)
#   6. (可選) 建立 systemd service
#   7. (可選) 建立桌面捷徑
#
# 用法：
#   curl -fsSL https://raw.githubusercontent.com/iOfficeAI/RickyTicky-BobbyWobbin/main/scripts/install-ubuntu.sh | bash
#   # 或指定版本：
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║          RickyTicky BobbyWobbin Installer for Ubuntu         ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
resolve_version() {
    if [[ -n "${AIONUI_VERSION:-}" ]]; then
        ...
        if command -v curl &>/dev/null; then
            VERSION=$(curl -fsSL "https://api.github.com/repos/iOfficeAI/RickyTicky-BobbyWobbin/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        elif command -v wget &>/dev/null; then
            VERSION=$(wget -qO- "https://api.github.com/repos/iOfficeAI/RickyTicky-BobbyWobbin/releases/latest" \
                | grep '"tag_name"' | head -1 | sed 's/.*"v\([^"]*\)".*/\1/')
        else
    ...
    DEB_FILENAME="RickyTicky BobbyWobbin-${VERSION}-linux-${DEB_ARCH}.deb"
    DOWNLOAD_URL="https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases/download/v${VERSION}/${DEB_FILENAME}"
}
...
# ─── 安裝 .deb + 修復依賴 ────────────────────────────────────────────────────
install_deb() {
    info "安裝 RickyTicky BobbyWobbin .deb 套件..."
    ...
    success "RickyTicky BobbyWobbin v${VERSION} 安裝完成"

    # 驗證安裝
    if command -v RickyTicky BobbyWobbin &>/dev/null || [[ -x /usr/bin/RickyTicky BobbyWobbin ]]; then
        success "RickyTicky BobbyWobbin 已安裝至 $(which RickyTicky BobbyWobbin 2>/dev/null || echo '/usr/bin/RickyTicky BobbyWobbin')"
    else
        warn "安裝可能不完整，找不到 RickyTicky BobbyWobbin 執行檔"
    fi
...
create_service_script() {
    local script_dir="/opt/RickyTicky BobbyWobbin"
...
#!/bin/bash
# ============================================================================
# RickyTicky BobbyWobbin WebUI Headless 服務管理腳本
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "⚡ RickyTicky BobbyWobbin 已在執行中 (PID: $(cat "$PIDFILE"))"
        return 1
    fi

    echo "🚀 正在啟動 RickyTicky BobbyWobbin WebUI..."
    cd "$WORKDIR" || exit 1
    ...
    if kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ RickyTicky BobbyWobbin 啟動成功 (PID: $(cat "$PIDFILE"))"
        local ip
        ip=$(hostname -I 2>/dev/null | awk '{print $1}')
        echo "🌐 WebUI: http://${ip:-localhost}:25808"
    else
        echo "❌ RickyTicky BobbyWobbin 啟動失敗，請查看日誌: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "⚠️  RickyTicky BobbyWobbin 未在執行"
        return 1
    fi
    local pid
    pid=$(cat "$PIDFILE")
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat "$PIDFILE")" 2>/dev/null; then
        echo "✅ RickyTicky BobbyWobbin 執行中 (PID: $(cat "$PIDFILE"))"
...
case "${1:-}" in
    ...
    "")
        ...
        echo ""
        echo "環境變數:"
        echo "  AIONUI_WORKDIR  - RickyTicky BobbyWobbin 工作目錄 (預設: \$HOME)"
        ;;
    *)
        echo "用法: $0 {start|stop|restart|status|logs}"
        exit 1
...
[Unit]
Description=RickyTicky BobbyWobbin AI Agent Desktop App (WebUI Mode)
Documentation=https://github.com/iOfficeAI/RickyTicky-BobbyWobbin
...
ExecStart=/usr/bin/xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" /usr/bin/RickyTicky BobbyWobbin --webui --remote --no-sandbox
Restart=on-failure
RestartSec=10
...
Exec=/usr/bin/RickyTicky BobbyWobbin --no-sandbox %U
Icon=RickyTicky BobbyWobbin
Terminal=false
Type=Application
Categories=Office;Utility;Development;
MimeType=x-scheme-handler/aionui;
StartupWMClass=RickyTicky BobbyWobbin
DESKTOP_EOF
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 RickyTicky BobbyWobbin v${VERSION} 安裝完成！${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 執行檔位置:${NC}  /usr/bin/RickyTicky BobbyWobbin"
    echo -e "  ${BOLD}📍 管理腳本:${NC}    /opt/RickyTicky BobbyWobbin/start-aionui.sh"
    echo ""
    ...
    if [[ "${MODE}" == "headless" ]]; then
        ...
        echo ""
        echo "    # 使用管理腳本"
        echo "    /opt/RickyTicky BobbyWobbin/start-aionui.sh start"
        echo "    /opt/RickyTicky BobbyWobbin/start-aionui.sh status"
        echo "    /opt/RickyTicky BobbyWobbin/start-aionui.sh stop"
        echo ""
        if command -v systemctl &>/dev/null; then
            echo "    # 或使用 systemd"
        ...
        echo ""
        echo "    # 直接啟動（桌面環境）"
        echo "    RickyTicky BobbyWobbin --no-sandbox"
        echo ""
        echo "    # 或從應用程式選單尋找 RickyTicky BobbyWobbin"
        echo ""
    fi

    echo -e "  ${BOLD}📖 文件:${NC}  https://github.com/iOfficeAI/RickyTicky-BobbyWobbin"
    echo -e "  ${BOLD}🐛 回報:${NC}  https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues"
    echo ""
