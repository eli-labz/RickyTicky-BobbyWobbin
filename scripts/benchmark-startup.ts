 ...
 * parsing the electron-log file for [RickyTicky BobbyWobbin:ready] / [RickyTicky BobbyWobbin:init] /
 * [RickyTicky BobbyWobbin:process] marks, plus `ready-to-show` / `did-finish-load` /
...
type StartupTiming = {
  ...
  wallTotalMs: number;
  // Parsed from [RickyTicky BobbyWobbin:ready] marks
  ...
  // Parsed from [RickyTicky BobbyWobbin:init] marks
  initTotalMs: number;
  // Parsed from [RickyTicky BobbyWobbin:process] marks
...
function getLogFilePath(): string {
  ...
  if (process.platform === 'darwin') {
    candidates.push(
      path.join(os.homedir(), 'Library', 'Logs', 'RickyTicky BobbyWobbin-Dev', `${today}.log`),
      path.join(os.homedir(), 'Library', 'Logs', 'RickyTicky BobbyWobbin', `${today}.log`)
    );
  } else if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(os.homedir(), 'AppData', 'Roaming');
    candidates.push(path.join(appData, 'RickyTicky BobbyWobbin', 'logs', `${today}.log`));
  } else {
    candidates.push(path.join(os.homedir(), '.config', 'RickyTicky BobbyWobbin', 'logs', `${today}.log`));
  }
...
// Matches: [RickyTicky BobbyWobbin:ready] <label> +<ms>ms
// Matches: [RickyTicky BobbyWobbin:init]  <label> +<ms>ms
// Matches: [RickyTicky BobbyWobbin:process] <label> +<ms>ms
const MARK_REGEX = /\[RickyTicky BobbyWobbin:(ready|init|process)\]\s+([^+]+?)\s+\+(\d+)ms/;
...
function parseStartupLog(lines: string[]): ParsedMarks {
  ...
  for (const line of lines) {
    ...
    if (line.includes('[RickyTicky BobbyWobbin] Renderer did-finish-load')) marks.logs.rendererDidFinishLoad = true;
    else if (line.includes('[RickyTicky BobbyWobbin] Window ready-to-show')) marks.logs.windowReadyToShow = true;
    else if (line.includes('[RickyTicky BobbyWobbin] Showing main window')) marks.logs.showingMainWindow = true;
  }
