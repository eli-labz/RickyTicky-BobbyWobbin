...
RickyTicky BobbyWobbin development uses two repositories:
...
- **RickyTicky BobbyWobbin** (`https://github.com/iOfficeAI/RickyTicky-BobbyWobbin.git`) starts the Electron desktop app and launches the backend binary automatically.
...
The desktop development server resolves the backend from the `PATH` inherited by `bun run start`. Install AionCore first, verify the binary is discoverable in the same terminal, then start RickyTicky BobbyWobbin.
...
git clone https://github.com/iOfficeAI/RickyTicky-BobbyWobbin.git
```
...
# Verify that RickyTicky BobbyWobbin will be able to find the backend.
which aioncore
aioncore --help
```
...
### 3. Start RickyTicky BobbyWobbin

Run these commands from the `RickyTicky BobbyWobbin` repository in a terminal where `aioncore` is discoverable.

```bash
cd RickyTicky BobbyWobbin
...
During startup, RickyTicky BobbyWobbin launches `aioncore` automatically and passes the backend port to the renderer. You do not need to start AionCore in a separate terminal.

## Updating the Local Backend

When you pull or change AionCore, reinstall the backend binary and restart RickyTicky BobbyWobbin:
...
RickyTicky BobbyWobbin cannot find the backend from the `PATH` inherited by `bun run start`.

Check from the same terminal where you start RickyTicky BobbyWobbin:
...
If the command fails, add Cargo's binary directory to `PATH` and start RickyTicky BobbyWobbin from a new terminal.

### `aioncore` Works in a Terminal but RickyTicky BobbyWobbin Still Cannot Find It
...
Quit RickyTicky BobbyWobbin, reinstall AionCore with `cargo install --path crates/aionui-app --locked --force`, then start RickyTicky BobbyWobbin again. The Electron app owns the backend subprocess during development, so a running RickyTicky BobbyWobbin instance will not pick up a newly installed binary until it restarts.
...
When you have two clones of the repository (e.g. `RickyTicky BobbyWobbin` and `RickyTicky BobbyWobbin-refactor`) and need to run both simultaneously, the second instance can be started with:
...
- Uses a separate userData directory (`RickyTicky BobbyWobbin-Dev-2`) to avoid database and config conflicts
...
RickyTicky BobbyWobbin uses **electron-vite** for fast bundling:
