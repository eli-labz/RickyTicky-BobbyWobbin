...
1. 用户打开「设置 → 关于」页面
2. 页面顶部居中显示应用名 "RickyTicky BobbyWobbin"（h3 标题）
3. 下方显示应用描述（通过 i18n 系统，随语言设置变化）
4. 显示当前版本号 badge（格式 `v{x.y.z}`），版本号来自打包时的 `package.json`
5. 版本号旁有 GitHub 图标，点击在系统浏览器中打开项目仓库 `https://github.com/iOfficeAI/RickyTicky-BobbyWobbin`
...
| 序号 | 标题     | 行为               | 目标                                           |
| ---- | -------- | ------------------ | ---------------------------------------------- |
| 1    | 帮助文档 | 打开外部链接       | `https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/wiki`     |
| 2    | 更新日志 | 打开外部链接       | `https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases` |
| 3    | 意见反馈 | 打开外部链接       | `https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues`   |
...
- Missing `RickyTicky BobbyWobbin.exe`, core DLLs, or `resources\app.asar` fails through `AIONUI_FAIL_UX` with code `E1031` and logs the missing label and path.
- Bundled AionCore verification remains `E1030`.
- If a silent `--updated` install cannot close RickyTicky BobbyWobbin after retries, the installer writes `%APPDATA%\RickyTicky BobbyWobbin\installer-last-failure.json`.
...
```json
{
  ...
  "updated": true,
  "retryCount": 3,
  "instDir": "C:\\Program Files\\RickyTicky BobbyWobbin",
...
- Run silent `/S --updated` while RickyTicky BobbyWobbin cannot be closed. Expected: marker file appears under `%APPDATA%\RickyTicky BobbyWobbin`; next app launch shows the update failure notification; the following launch does not show it again.
