# RickyTicky BobbyWobbin Headless Server Deployment Guide

Deploy RickyTicky BobbyWobbin WebUI on headless Linux servers — cloud VMs, Kubernetes Pods, and containers — with proxy auto-fallback support.
...
- At least 2GB RAM
- RickyTicky BobbyWobbin `.deb` package from [Releases](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases)
...
wget https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases/latest/download/RickyTicky BobbyWobbin-linux-amd64.deb

# Install
sudo dpkg -i RickyTicky BobbyWobbin-linux-amd64.deb
...
RickyTicky BobbyWobbin is an Electron app and requires a display server. On headless servers (no monitor), use Xvfb to create a virtual display:
...
Create `/opt/RickyTicky BobbyWobbin/start-aionui.sh`:

```bash
#!/bin/bash
# RickyTicky BobbyWobbin WebUI headless startup script
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "RickyTicky BobbyWobbin is already running (PID: $(cat $PIDFILE))"
        return 1
    fi
    ...
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/RickyTicky BobbyWobbin --webui --remote --no-sandbox \
        > "$LOGFILE" 2>&1 &
    echo $! > "$PIDFILE"
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "RickyTicky BobbyWobbin started successfully (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "RickyTicky BobbyWobbin failed to start. Check log: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "RickyTicky BobbyWobbin is not running (no PID file)"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "Stopping RickyTicky BobbyWobbin (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "RickyTicky BobbyWobbin --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "RickyTicky BobbyWobbin stopped."
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "RickyTicky BobbyWobbin is running (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "RickyTicky BobbyWobbin is not running."
...
```bash
chmod +x /opt/RickyTicky BobbyWobbin/start-aionui.sh
```

> **Tip**: `WORKDIR` determines the directory RickyTicky BobbyWobbin can access for file operations. Set it to your project workspace.

---

## Remote Access

RickyTicky BobbyWobbin WebUI listens on port **25808**. Choose a method based on your network setup:
...
### Step 2: PAC File for RickyTicky BobbyWobbin (Electron / Chromium Layer)
...
Create `/opt/RickyTicky BobbyWobbin/proxy.pac`:
...
```bash
    nohup xvfb-run --auto-servernum --server-args="-screen 0 1920x1080x24" \
        /usr/bin/RickyTicky BobbyWobbin --webui --remote --no-sandbox \
        --proxy-pac-url="file:///opt/RickyTicky BobbyWobbin/proxy.pac" \
        > "$LOGFILE" 2>&1 &
```
...
### Step 4: RickyTicky BobbyWobbin Internal Proxy (Gemini API)

For Gemini API calls, configure the proxy inside RickyTicky BobbyWobbin WebUI:

**Settings → Gemini Settings → Proxy** → `http://127.0.0.1:7897`

> This proxy is handled by RickyTicky BobbyWobbin's Node.js layer (separate from the Chromium layer). When the SSH tunnel is down, Gemini API calls will fail, but the WebUI and other APIs remain functional.
...
| Issue                                     | Solution                                                     |
| ----------------------------------------- | ------------------------------------------------------------ |
| `dpkg` dependency errors in containers    | `dpkg --force-all -i RickyTicky BobbyWobbin-linux-amd64.deb`                 |
| RickyTicky BobbyWobbin can only access `/tmp`             | Set `WORKDIR` in the startup script to your workspace path   |
...
│       ▼                                          │
│  ┌────────────────────────────┐                  │
│  │  RickyTicky BobbyWobbin (Electron)        │                   │
│  │  ├─ Chromium (port 25808) │                   │
...
# RickyTicky BobbyWobbin 无头服务器部署指南

在无图形界面的 Linux 服务器（云主机、K8s Pod、容器）上部署 RickyTicky BobbyWobbin WebUI，支持代理自动回退。

## 前置条件

- Linux x86_64（推荐 Ubuntu 20.04+ / Debian 11+）
- 至少 2GB 内存
- RickyTicky BobbyWobbin `.deb` 安装包（[下载地址](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases)）
...
RickyTicky BobbyWobbin 是 Electron 应用，需要显示服务。无头服务器需安装 Xvfb：
...
创建 `/opt/RickyTicky BobbyWobbin/start-aionui.sh`：

```bash
#!/bin/bash
# RickyTicky BobbyWobbin WebUI 无头启动脚本
...
start() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "RickyTicky BobbyWobbin 已在运行 (PID: $(cat $PIDFILE))"
        return 1
    fi
    echo "正在启动 RickyTicky BobbyWobbin WebUI..."
    cd "$WORKDIR"
    ...
    sleep 3
    if kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "RickyTicky BobbyWobbin 启动成功 (PID: $(cat $PIDFILE))"
        echo "WebUI: http://$(hostname -I | awk '{print $1}'):25808"
    else
        echo "RickyTicky BobbyWobbin 启动失败，请查看日志: $LOGFILE"
        rm -f "$PIDFILE"
        return 1
    fi
}
...
stop() {
    if [ ! -f "$PIDFILE" ]; then
        echo "RickyTicky BobbyWobbin 未在运行"
        return 1
    fi
    PID=$(cat "$PIDFILE")
    echo "正在停止 RickyTicky BobbyWobbin (PID: $PID)..."
    kill "$PID" 2>/dev/null
    sleep 2
    kill -9 "$PID" 2>/dev/null
    pkill -f "RickyTicky BobbyWobbin --webui" 2>/dev/null
    rm -f "$PIDFILE"
    echo "RickyTicky BobbyWobbin 已停止。"
}
...
status() {
    if [ -f "$PIDFILE" ] && kill -0 "$(cat $PIDFILE)" 2>/dev/null; then
        echo "RickyTicky BobbyWobbin 运行中 (PID: $(cat $PIDFILE))"
        ss -tlnp | grep 25808
    else
        echo "RickyTicky BobbyWobbin 未在运行。"
        rm -f "$PIDFILE" 2>/dev/null
...
RickyTicky BobbyWobbin WebUI 监听端口 **25808**，根据网络环境选择访问方式：
...
### 第二步：PAC 代理文件（RickyTicky BobbyWobbin Electron 层）

`--proxy-server` 的问题：代理一断，**所有请求**全挂。改用 PAC 文件实现自动回退。

创建 `/opt/RickyTicky BobbyWobbin/proxy.pac`：
...
启动脚本中添加参数：`--proxy-pac-url="file:///opt/RickyTicky BobbyWobbin/proxy.pac"`
...
### 第四步：RickyTicky BobbyWobbin 内置代理（Gemini API）
...
| RickyTicky BobbyWobbin 只能访问 /tmp   | 修改启动脚本中的 `WORKDIR`            |
