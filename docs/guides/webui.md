# RickyTicky BobbyWobbin WebUI Mode - Startup Guide

RickyTicky BobbyWobbin supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts RickyTicky BobbyWobbin with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use RickyTicky BobbyWobbin from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
   "C:\Program Files\RickyTicky BobbyWobbin\RickyTicky BobbyWobbin.exe" --webui

# Or if RickyTicky BobbyWobbin is in your PATH
RickyTicky BobbyWobbin.exe --webui
```
...
2. Enter target location:
   ...
   "C:\Program Files\RickyTicky BobbyWobbin\RickyTicky BobbyWobbin.exe" --webui
   ```
3. Name it **RickyTicky BobbyWobbin WebUI**
4. Click **Finish**
...
```bash
# Using full path
/Applications/RickyTicky BobbyWobbin.app/Contents/MacOS/RickyTicky BobbyWobbin --webui

# Or using open command
open -a RickyTicky BobbyWobbin --args --webui
```
...
4. Enter:
   ...
   /Applications/RickyTicky BobbyWobbin.app/Contents/MacOS/RickyTicky BobbyWobbin --webui
   ```
5. Save as **RickyTicky BobbyWobbin WebUI.app**
6. Double-click to launch

### Method 4: Add to Dock

1. Create an Automator app (Method 3)
2. Drag **RickyTicky BobbyWobbin WebUI.app** to your Dock
...
# Or using full path
/opt/RickyTicky BobbyWobbin/aionui --webui
```
...
chmod +x RickyTicky BobbyWobbin-*.AppImage

# Run with --webui flag
./RickyTicky BobbyWobbin-*.AppImage --webui
```
...
```ini
[Desktop Entry]
Name=RickyTicky BobbyWobbin WebUI
Comment=Start RickyTicky BobbyWobbin in WebUI mode
Exec=/opt/RickyTicky BobbyWobbin/aionui --webui
Icon=aionui
Terminal=false
Type=Application
...
```ini
[Unit]
Description=RickyTicky BobbyWobbin WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/RickyTicky BobbyWobbin/aionui --webui --remote
Restart=on-failure
RestartSec=10
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run RickyTicky BobbyWobbin in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making RickyTicky BobbyWobbin accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running RickyTicky BobbyWobbin WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues/217)
...
#### 4. Download and Install RickyTicky BobbyWobbin

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases
wget https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases/download/vVERSION/RickyTicky BobbyWobbin_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/releases/download/vVERSION/RickyTicky BobbyWobbin_VERSION_arm64.deb

# Install the package
apt install -y ./RickyTicky BobbyWobbin_*.deb

# Verify installation
which RickyTicky BobbyWobbin
```

#### 5. Launch RickyTicky BobbyWobbin WebUI

```bash
# Start RickyTicky BobbyWobbin in WebUI mode with no-sandbox flag
RickyTicky BobbyWobbin --no-sandbox --webui
```
...
To access RickyTicky BobbyWobbin from other devices on your local network:

```bash
# Start with --remote flag
RickyTicky BobbyWobbin --no-sandbox --webui --remote
...
RickyTicky BobbyWobbin --no-sandbox --webui --port 8080
```
...
chmod +x /opt/RickyTicky BobbyWobbin/aionui
```

#### Out of Memory

RickyTicky BobbyWobbin requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if RickyTicky BobbyWobbin is running: look for "Server started" message
...
4. **Keep device charged** - Running RickyTicky BobbyWobbin consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **RickyTicky BobbyWobbin Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch RickyTicky BobbyWobbin quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-aionui.sh << 'EOF'
#!/bin/bash
echo "Starting RickyTicky BobbyWobbin WebUI..."
RickyTicky BobbyWobbin --no-sandbox --webui --remote
EOF
...
```bash
proot-distro login ubuntu -- bash -c "RickyTicky BobbyWobbin --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues/217)
...
```cmd
RickyTicky BobbyWobbin.exe --webui --remote
```

### macOS

```bash
/Applications/RickyTicky BobbyWobbin.app/Contents/MacOS/RickyTicky BobbyWobbin --webui --remote
```
...
netsh advfirewall firewall add rule name="RickyTicky BobbyWobbin WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add RickyTicky BobbyWobbin
...
```cmd
where RickyTicky BobbyWobbin.exe
```

**macOS:**

```bash
mdfind -name "RickyTicky BobbyWobbin.app"
```
...
```powershell
& "C:\Program Files\RickyTicky BobbyWobbin\RickyTicky BobbyWobbin.exe" --webui 2>&1 | Tee-Object -FilePath aionui.log
```
...
| Platform | Location                                                 |
| -------- | -------------------------------------------------------- |
| Windows  | `%APPDATA%/RickyTicky BobbyWobbin/webui.config.json`                     |
| macOS    | `~/Library/Application Support/RickyTicky BobbyWobbin/webui.config.json` |
| Linux    | `~/.config/RickyTicky BobbyWobbin/webui.config.json`                     |
...
```cmd
# Using full path
"C:\Program Files\RickyTicky BobbyWobbin\RickyTicky BobbyWobbin.exe" --resetpass

# Or for a specific user
"C:\Program Files\RickyTicky BobbyWobbin\RickyTicky BobbyWobbin.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/RickyTicky BobbyWobbin.app/Contents/MacOS/RickyTicky BobbyWobbin --resetpass

# Or for a specific user
/Applications/RickyTicky BobbyWobbin.app/Contents/MacOS/RickyTicky BobbyWobbin --resetpass username
```
...
# Or using full path
/opt/RickyTicky BobbyWobbin/aionui --resetpass
```
...
- [GitHub Issues](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues)
...
2. Search [existing issues](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues)
3. Create a [new issue](https://github.com/iOfficeAI/RickyTicky-BobbyWobbin/issues/new) with:
   - Your OS and version
   - RickyTicky BobbyWobbin version
   - Steps to reproduce
   - Error messages or logs

---

**Happy using RickyTicky BobbyWobbin in WebUI mode!** 🚀
